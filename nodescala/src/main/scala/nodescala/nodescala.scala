...
trait NodeScala {
   ...
   */
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = async {
...
object NodeScala {
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      val handler: Exchange => Unit = { exchange =>
        p.trySuccess(exchange.request, exchange)
        removeContext()
      }
